...
import EditXivDaysModal from "./EditXivDaysModal"
import EditXiiiModal from "./EditXiiiModal"
...
export default async function PayrollDetailsPage({ params }: { params: { id: string } }) {
    ...
    const isXiv = (period as any).type === 'DECIMO_CUARTO_COSTA' || (period as any).type === 'DECIMO_CUARTO_SIERRA'
    const isXIII = (period as any).type === 'DECIMO_TERCERO'
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    {!isClosed && period.status === 'processed' && (
                        <form action={async () => {
                            'use server'
                            if (isXIII) {
                                const { recalculateXiiiPayroll } = await import('../actions')
                                await recalculateXiiiPayroll(period.id)
                            } else if (isXiv) {
                                // XIV se recalcula automáticamente al editar días
                                // No necesita botón de recalcular
                            } else {
                            ...
                            <button className="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                                <RefreshCw size={14} />
                                {isXIII ? 'Recalcular XIII' : 'Recalcular Todo'}
                            </button>
                        </form>
                    )}
                    {!isClosed && period.status === 'processed' && (
                    ...
                    <table className="w-full text-left text-sm text-gray-500 min-w-[600px]">
                        <thead className="bg-gray-50 text-xs uppercase text-gray-700">
                            <tr>
                                ...
                                <th className="px-6 py-4">XIV (SBU/360 × Días)</th>
                                <th className="px-6 py-4">Deducciones</th>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {period.records.length === 0 ? (
                                ...
                                const xivAmount = record.benefits.find((b: any) => b.type === 'DECIMO_CUARTO')?.amount || 0;
                                    ...
                                    <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            {isClosed ? (
                                                ...
                                                <EditXivDaysModal recordId={record.id} currentDays={record.daysWorked} />
                                            )}
                                        </td>
                                        <td className="px-6 py-4 text-green-600 font-semibold">+${xivAmount.toFixed(2)}</td>
                                        <td className="px-6 py-4">
                                            ...
                                            {additionalDeductions.length > 0 && (
                                                <div className="flex flex-wrap gap-1 mt-1">
                                                    {additionalDeductions.map((d: any) => (
                                                        <span key={d.id} className="inline-flex items-center gap-1 rounded bg-red-50 px-1.5 py-0.5 text-red-700 border border-red-100 text-[10px]">
                ...
                <div className="rounded-xl border bg-white shadow-sm overflow-x-auto">
                    <table className="w-full text-left text-sm text-gray-500 min-w-[600px]">
                        <thead className="bg-gray-50 text-xs uppercase text-gray-700">
                            <tr>
                                <th className="px-6 py-4">Empleado</th>
                                <th className="px-6 py-4">XIII (Ingresos IESSables / 12)</th>
                                <th className="px-6 py-4">Deducciones</th>
                                <th className="px-6 py-4 text-right">Neto a Recibir</th>
                                <th className="px-6 py-4 text-center">Acciones</th>
                            </tr>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {period.records.length === 0 ? (
                                ...
                                const additionalDeductions = record.deductions.filter((d: any) => d.deductionType);
                                const xiiiAmount = record.benefits.find((b: any) => b.type === 'DECIMO_TERCERO')?.amount || 0;
                                ...
                                return (
                                    <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            {isClosed ? (
                                                <span className="text-green-600 font-semibold">+${xiiiAmount.toFixed(2)}</span>
                                            ) : (
                                                <EditXiiiModal recordId={record.id} currentAmount={xiiiAmount} />
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="text-red-600 font-semibold">-${record.totalDeductions.toFixed(2)}</div>
